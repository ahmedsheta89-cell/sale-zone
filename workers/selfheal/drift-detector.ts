...
function resolveDefaultContractPath(): string {
  const fromRepoRoot = path.resolve(process.cwd(), 'workers/contracts/queue-contract.json');
  if (fs.existsSync(fromRepoRoot)) return fromRepoRoot;
  return path.resolve(process.cwd(), 'contracts/queue-contract.json');
}

export async function detectTopologyDrift(contractPath = resolveDefaultContractPath()): Promise<{ drift: boolean; reasons: string[] }> {
