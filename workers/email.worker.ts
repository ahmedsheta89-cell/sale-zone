...
export async function bootEmailWorker(): Promise<RuntimeHandle> {
  const queueName = 'prod:email';
  const stopHeartbeat = await startHeartbeat(`email-${process.pid}`, 30000);
  ...
  const worker = await createWorker(queueName, async (job: Job) => {
    ...
    if (parsed.jobType !== 'email.send') throw new Error('invalid_job_type_for_email_worker');
    const started = Date.now();
    await new Promise((r) => setTimeout(r, 20));
    emitMetric('processing_latency_ms', Date.now() - started, { queue: queueName });
    log('info', 'email processed', { jobId: job.id });
    return { ok: true };
  }, 5);
  let closed = false;
  return {
